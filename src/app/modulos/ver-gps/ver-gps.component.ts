...
export class VerGpsComponent implements OnInit {
  ...
  abrirDialogoEliminar(data:Gps) {
    this.eliminar=true;
    this.editar=false;
    this.title="¿Esta seguro que desea deshabilitar el siguiente gps?"
  ...
  abrirDialogoActivar(data:Gps){
    this.eliminar=false;
    this.editar=true;
    this.title="¿Desea hablitar nuevamente el siguiente gps?"
    this.gpsElimnar=data;
    this.dialog.open(this.eliminarGps);
  }
  ...
  EliminarGps(){
    ...
    this.servicegps.editGps(this.gpsElimnar,this.gpsElimnar.id_gps).subscribe(value => {
      this.snackBar.open("El GPS CON IMEI "+this.gpsElimnar.imei_gps+" HA SIDO DESHABILITADO", "",{
        duration: 1 * 1000,
      });
    })
  }
